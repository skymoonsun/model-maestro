...
import { bedrockModelsApi, nodesApi, type VllmModel } from '@/lib/api';
...
export default function BedrockModelsPage() {
    const [search, setSearch] = useState('');
    const queryClient = useQueryClient();
    const { data: models, isLoading } = useQuery({
        queryKey: ['bedrock-models'],
        queryFn: bedrockModelsApi.list,
    });
    ...
    const toggleMutation = useMutation({
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['bedrock-models'] });
                                    ...
                                    <TableHead className="text-right">
                                        <div className="inline-flex items-center justify-end gap-2 w-full">
                                            ...
                                            <BulkAvailableSwitch
                                                ...
                                                invalidateQueryKeys={[['bedrock-models'], ['nodes']]}
                                            />
                                        </div>
                                    </TableHead>
                                </TableRow>
                                ...
                                {groupedList.length === 0 && (
                                    <TableRow>
                                        <TableCell colSpan={7} className="text-center text-muted-foreground py-12">
                                            {search ? 'No results found' : 'No Bedrock models found. Add a Bedrock node and run model sync to discover models.'}
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
