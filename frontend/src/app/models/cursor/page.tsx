...
import { cursorModelsApi, nodesApi } from '@/lib/api';
...
export default function CursorModelsPage() {
    const [search, setSearch] = useState('');
    const queryClient = useQueryClient();
    const { data: models, isLoading } = useQuery({
        queryKey: ['cursor-models'],
        queryFn: cursorModelsApi.list,
    });
                                    ...
                                    <TableHead className="text-right">
                                        <div className="inline-flex items-center justify-end gap-2 w-full">
                                            ...
                                            <BulkAvailableSwitch
                                                ...
                                                invalidateQueryKeys={[['cursor-models'], ['nodes']]}
                                            />
                                        </div>
                                    </TableHead>
                                </TableRow>
                                ...
                                {groupedList.length === 0 && (
                                    <TableRow>
                                        <TableCell colSpan={7} className="text-center text-muted-foreground py-12">
                                            {search ? 'No results found' : 'No cursor models found. Add a cursor node and run model sync to discover models.'}
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
