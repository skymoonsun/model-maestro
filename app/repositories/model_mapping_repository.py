...
class ModelMappingRepository:
    ...
    async def create(
        ...
        await self.sync_mapping_nodes(mapping, node_ids)
    ...
    async def upsert(
        ...
        if existing:
            ...
            if node_ids is not None:
                await self.sync_mapping_nodes(existing, node_ids)
            await self.session.commit()
            await self.session.refresh(existing)
            return existing, False
        mapping = await self.create(display_name, real_name, node_ids, context_length, capabilities)
        return mapping, True
    ...
    async def get_by_display_name(self, display_name: str) -> Optional[ModelMapping]:
        ...
        result = await self.session.execute(
            select(ModelMapping)
            .options(selectinload(ModelMapping.nodes))
            .where(ModelMapping.display_name == display_name)
        )
    ...
    async def list_all(self) -> List[ModelMapping]:
        """List all mappings"""
        result = await self.session.execute(
            select(ModelMapping).options(selectinload(ModelMapping.nodes))
        )
